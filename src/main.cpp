...
public:
    ...
    ~SSLManager() { if (ctx) SSL_CTX_free(ctx); }
...
public:
    ...
    ~Server() { if (server_fd != INVALID_SOCKET) closesocket(server_fd); }
    ...
    bool init() {
...
#endif
        if (!sslManager.init()) return false;
        ...
        if (bind(server_fd, (struct sockaddr*)&server_addr, sizeof(server_addr)) == SOCKET_ERROR ||
            listen(server_fd, SOMAXCONN) == SOCKET_ERROR) return false;
    ...
    void run() {
        ...
        while (server_running) {
            ...
            std::thread client_thread([ssl, client_addr, this]() { ClientHandler::handle(ssl, client_addr, router); });
...
int main() {
    ...
    //server.register_route("GET", "/", [](const std::string&) { return "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\nWelcome!"; });
    server.register_route("POST", "/register", [](const std::string& body) { return "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\nUser Registered: " + body; });
    server.run();
    return 0;
}
